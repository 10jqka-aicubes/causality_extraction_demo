...
class ReduceLROnPlateau(object):
    ...
    def __init__(
        self, optimizer, mode="min", factor=0.1, patience=10, verbose=0, epsilon=1e-4, cooldown=0, min_lr=0, eps=1e-8
    ):
...
class CosineLRWithRestarts(object):
    ...
    def get_lr(self, t_cur):
        ...
        eta_t = eta_min + 0.5 * (eta_max - eta_min) * (1.0 + math.cos(math.pi * (t_cur / self.restart_period)))
        ...
        weight_decays = [
            base_weight_decay * eta_t * weight_decay_norm_multi for base_weight_decay in self.base_weight_decays
        ]
